...
import Base: axes, getindex, size, setindex!, IndexStyle, IdentityUnitRange, Slice
...
function frames_min_max_mean(::Type{S}, ::Type{T}, imgs; kwargs...) where {S,T}
    nf = size(imgs, 3)
    nf > 0 || throw(ArgumentError("imgs must not be empty"))
...
larger_container_type(::Type{Union{}}, ::Type{T}) where {T} = T
larger_container_type(::Type{T}, ::Type{Union{}}) where {T} = T
larger_container_type(::Type{Union{}}, ::Type{Union{}}) = Union{}
...
larger_container_rule(::Type{<:Any}, ::Type{<:Any}) = Union{}
...
function frame_map!(
    ...
    if nt > 1
        ...
        colrange = 1:nc
        for fno = 1:nf
            _frame_pixel_map!(pixel_f, dest, stack, fno, colrange, rowrange, dest)
        end
    end
    dest
end
...
function frame_sink_map!(
    ...
    if nt > 1
        ...
        colrange = 1:nc
        for fno = 1:nf
            _frame_pixel_map!(pixel_f, dest, stack, fno, colrange, rowrange, args...)
            frame_sink_f(dest, fno)
        end
    end
    nothing
...
function stack_map!(
    ...
    if nt > 1
        ...
        colrange = 1:nc
        for fno = 1:nf
            _stack_map!(pixel_f, dest, stack, fno, colrange, rowrange, args...)
        end
    end
    nothing
...
function median_filter_frames!(
    ...
    nx, ny, nf = size(inb)
    size(filterbank) == (nx, ny) || throw(ArgumentError("filter size does not match"))
